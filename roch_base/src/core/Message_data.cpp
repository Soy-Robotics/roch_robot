...
//    stream << "Right Integral Limit: " << getRightIntegralLimit() << endl;
    ...
    for (int i = 0; i < num_bat; ++i)
    {
      ...
      switch (getDescription(0).getType())
      {
        ...
        case BatteryDescription::NIMH:
          stream << "NiMH" << endl;
          break;
	case BatteryDescription::Li_ion:
	  stream << "Li_ion" << endl;
	  break;
  ...
  double DataRangefinders::getDistance(int rangefinder)
  {
    return btof(getPayloadPointer(1 + 2 * rangefinder), 2,1000);
  }
  ...
  double DataRangefinderTimings::getDistance(int rangefinder)
  {
    return btof(getPayloadPointer(1 + 2 * rangefinder), 2,1000);
  }
  ...
  MESSAGE_CONSTRUCTORS(Data6AxisYaw, PAYLOAD_LEN)

  MESSAGE_CONVENIENCE_FNS(Data6AxisYaw, DATA_6AXIS_YAW)

  double Data6AxisYaw::getAngle()
  {
    return btof(getPayloadPointer(Angle), 2, 1000);
  }

  double Data6AxisYaw::getAngleRate()
  {
    return btof(getPayloadPointer(Angle_rate), 2, 1000);
  }

  ostream &Data6AxisYaw::printMessage(ostream &stream)
  {
    stream << "Raw Yaw Data" << endl;
    stream << "=====================" << endl;
    stream << "Angle: 0x" << hex << getAngle() << endl;
    stream << "Angle Rate: 0x" << getAngleRate() << endl;
    return stream;
  }
